...
export const captureOrder = async (req: Request, res: Response): Promise<void> => {
  ...
  const returnPath = service ? decodeURIComponent(service as string) : '/payment-success';
 ...
 */
export const verifyPaymentToken = async (req: Request, res: Response): Promise<Response> => {
  try {
